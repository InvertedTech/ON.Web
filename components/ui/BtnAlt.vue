<template>
  <nuxt-link v-if="to" :to="to" class="altbtn outline-none rounded-full shadow-md relative text-center" :disabled="disabled || loading" :class="classList">
    <slot />
  ...
  <button v-else class="altbtn outline-none rounded-full shadow-md relative" :disabled="disabled || loading" :type="type" :class="classList" @mousedown.prevent @click="click">
    <slot />
    <div v-if="loading" class="text-white absolute top-0 left-0 w-full h-full flex items-center justify-center text-opacity-100">
      <svg class="animate-spin" style="width: 24px; height: 24px" viewBox="0 0 24 24">
        <path fill="currentColor" d="M12,4V2A10,10 0 0,0 2,12H4A8,8 0 0,1 12,4Z" />
...
.altbtn:hover:not(:disabled)::before {
