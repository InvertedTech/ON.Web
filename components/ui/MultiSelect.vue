  ...
  <div class="w-full">
    ...
    <div ref="wrapper" class="relative">
      <div ref="inputWrapper" class="flex-wrap relative w-full shadow-sm flex items-center border border-stroke focus:border-grayscale-600 rounded-xl px-2 py-1 h-12" :class="wrapperClass" @click.stop.prevent="clickWrapper" @mouseup.stop.prevent @mousedown.prevent>
        <div v-for="item in selected" :key="item" class="rounded-full px-2 py-1 mx-0.5 my-0.5 text-xs bg-accent-darker flex flex-nowrap break-all items-center relative text-accent">
          <div v-if="!disabled" class="w-full h-full rounded-full absolute top-0 left-0 opacity-0 hover:opacity-100 px-1 bg-accent-darker bg-opacity-75 flex items-center justify-end">
            <span v-if="showEdit" class="material-icons text-accent hover:text-warning" style="font-size: 1.1rem" @click.stop="editItem(item)">edit</span>
            <span class="material-icons text-accent hover:text-red-500 cursor-pointer" style="font-size: 1.1rem" @click.stop="removeItem(item)">close</span>
          </div>
      ...
      <ul ref="menu" v-show="showMenu" class="absolute z-50 mt-1 w-full bg-grayscale-300 shadow-lg max-h-56 rounded-lg py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm" role="listbox" aria-labelledby="listbox-label">
        <template v-for="item in itemsToShow">
          <li :key="item" class="text-gray-50 select-none relative py-2 pr-9 cursor-pointer bg-grayscale-300 hover:bg-grayscale-400" role="option" @click="clickedOption($event, item)" @mouseup.stop.prevent @mousedown.prevent>
...
export default {
  ...
  computed: {
    ...
    showMenu() {
      return this.isFocused && (this.selected.length || this.showNoItems)
    },
    wrapperClass() {
      var classes = []
