<template>
  <div ref="wrapper" class="relative" v-click-outside="clickOutsideObj">
    <div ref="inputWrapper" class="w-8 h-8" :class="wrapperClass" @click.stop.prevent="clickWrapper" @mouseup.stop.prevent @mousedown.prevent>
      <div class="w-full h-full rounded" :style="{ backgroundColor: color }" />
    </div>

    <div ref="menu" v-if="showMenu" class="absolute z-50 mt-1 bg-grayscale-300 shadow-lg rounded-lg text-base focus:outline-none sm:text-sm">
      <ui-color-picker v-model="color" hide-swatches />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    wrapperClass() {
      if (!this.readonly && !this.disabled) return 'cursor-pointer'
      return ''
    }
  },
  methods: {
